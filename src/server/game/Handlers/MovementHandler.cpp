...
void WorldSession::HandleMoveWorldportAck()
{
    ...
    if (player->IsInWorld())
    {
        LOG_ERROR("network", "%s %s is still in world when teleported from map %s (%u) to new map %s (%u)", player->GetGUID().ToString().c_str(), player->GetName().c_str(), oldMap->GetMapName(), oldMap->GetId(), newMap ? newMap->GetMapName() : "Unknown", loc.GetMapId());
    ...
    {
        LOG_ERROR("network", "Map %d (%s) could not be created for player %s (%s), porting player to homebind", loc.GetMapId(), newMap ? newMap->GetMapName() : "Unknown", player->GetGUID().ToString().c_str(), player->GetName().c_str());
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    {
        LOG_WARN("misc", "The computed movement time using clockDelta is erronous. Using fallback instead");
...
void WorldSession::HandleForceSpeedChangeAck(WorldPacket &recvData)
{
    ...
    switch (recvData.GetOpcode())
    {
        ...
        default:
            LOG_ERROR("network", "WorldSession::HandleForceSpeedChangeAck: Unknown move type opcode: %u", recvData.GetOpcode());
            return;
    }
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPacket &recvData)
{
    ...
    if (GetPlayer()->IsInWorld())
        if (_player->GetUnitBeingMoved()->GetGUID() != guid)
            LOG_DEBUG("network", "HandleSetActiveMoverOpcode: incorrect mover guid: mover is %s and should be %s" , guid.ToString().c_str(), _player->GetUnitBeingMoved()->GetGUID().ToString().c_str());
}
...
{
    LOG_DEBUG("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
    ...
    if (!mover)
    {
        LOG_WARN("entities.player", "WorldSession::HandleMoveTimeSkippedOpcode wrong mover state from the unit moved by the player %s", GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (guid != mover->GetGUID())
    {
        LOG_WARN("entities.player", "WorldSession::HandleMoveTimeSkippedOpcode wrong guid from the unit moved by the player %s", GetPlayer()->GetGUID().ToString().c_str());
        return;
    }
