...
enum IPSShopType
{
    ...
    IPS_SHOP_TYPE_CHAR_CUSTOMIZE    // Character customize
};
...
private:
    ...
    {
        sMailExternal->AddMail(charName, thanksSubject, thanksText, 0, itemID, itemCount, 37688);
    }
    ...
    void SendRewardChangeCustomize(std::string const& charName)
    {
        auto targetGuid = sCharacterCache->GetCharacterGuidByName(charName);
        if (!targetGuid)
            return;
...
public:
    ...
    {
        sGameConfig->AddOption<bool>("IPSShop.Enable");
    }
