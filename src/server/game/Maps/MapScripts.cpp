...
inline GameObject* Map::_GetScriptGameObjectSourceOrTarget(Object* source, Object* target, ScriptInfo const* scriptInfo, bool bReverse) const
{
    ...
    else
    {
        ...
        if (!gameobject)
            LOG_ERROR("scripts", "%s neither source nor target are gameobjects (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
                scriptInfo->GetDebugInfo().c_str(),
                source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0, source ? source->GetGUID().GetCounter() : 0,
                target ? target->GetTypeId() : 0, target ? target->GetEntry() : 0, target ? target->GetGUID().GetCounter() : 0);
    }
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    if (!obj)
        ...
        LOG_ERROR("scripts", "%s %s object is not unit %s, skipping.",
...
inline Player* Map::_GetScriptPlayer(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        player = obj->ToPlayer();
        if (!player)
            LOG_ERROR("scripts", "%s %s object is not a player %s.",
                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetGUID().ToString().c_str());
    }
    return player;
}
...
inline Creature* Map::_GetScriptCreature(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!creature)
            LOG_ERROR("scripts", "%s %s object is not a creature %s.", scriptInfo->GetDebugInfo().c_str(),
...
inline WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!pWorldObject)
            LOG_ERROR("scripts", "%s %s object is not a world object %s.",
                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetGUID().ToString().c_str());
    }
    return pWorldObject;
}
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    if (!guid)
        ...
        LOG_ERROR("scripts", "%s source object is not unit %s, skipping.", scriptInfo->GetDebugInfo().c_str(),
            source->GetGUID().ToString().c_str());
    else
    {
        WorldObject* wSource = dynamic_cast <WorldObject*> (source);
        if (!wSource)
            LOG_ERROR("scripts", "%s source object could not be cast to world object %s, skipping.",
        ...
        else
        {
            ...
            if (!pDoor)
                ...
                LOG_ERROR("scripts", "%s gameobject is not a door (GoType: %u, %s).",
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_FLAG_SET:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    if (step.script->FlagToggle.FieldID <= OBJECT_FIELD_ENTRY || step.script->FlagToggle.FieldID >= cSource->GetValuesCount())
                        LOG_ERROR("scripts", "%s wrong field %u (max count: %u) in object %s specified, skipping.",
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (player)
                {
                    ...
                    {
                        LOG_ERROR("scripts", "%s source is not unit, gameobject or player %s, skipping.",
                ...
                else
                {
                    ...
                    if (player)
                    {
                        ...
                        {
                            LOG_ERROR("scripts", "%s target is not unit, gameobject or player %s, skipping.",
                    ...
                    {
                        LOG_ERROR("scripts", "%s neither source nor target is player (source: %s; target: %s), skipping.",
                            step.script->GetDebugInfo().c_str(), source->GetGUID().ToString().c_str(),
            ...
            case SCRIPT_COMMAND_ACTIVATE_OBJECT:
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    ...
                    {
                        LOG_ERROR("scripts", "%s target object is not gameobject %s, skipping.",
                            step.script->GetDebugInfo().c_str(), target->GetGUID().ToString().c_str());
                        break;
                    }
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                ...
                if (!uSource)
                {
                    LOG_ERROR("scripts", "%s no source worldobject found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
                    break;
                }

                if (!uTarget)
                {
                    LOG_ERROR("scripts", "%s no target worldobject found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
                    break;
                }
            ...
            case SCRIPT_COMMAND_KILL:
                ...
                {
                    if (cSource->isDead())
                        LOG_ERROR("scripts", "%s creature is already dead %s",
