...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    if (check.size() > 1)
    {
        ...
        {
            LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) No compatible dungeons%s", GetDetailedMatchRoles(check).c_str(), o.str().c_str());
    ...
    proposal.dungeonId = Server::Containers::SelectRandomContainerElement(proposalDungeons);
