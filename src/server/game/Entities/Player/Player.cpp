...
Player::Player(WorldSession* session): Unit(true)
{
    ...
    /////////////////// Custom /////////////////////
    m_isInGuildZone = false;
    m_guildZoneId = GUILD_ZONE_NONE;
}
...
{
    bool areaOnly = (m_zoneUpdateId == newZone);
    ...
    sScriptMgr->OnPlayerUpdateZone(this, newZone, newArea, areaOnly);
...
bool Player::LoadFromDB(ObjectGuid guid, SQLQueryHolder *holder)
{
    ...
    //"health, power1, power2, power3, power4, power5, power6, power7, instance_id, talentGroupsCount, activeTalentGroup, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, customBankMoney FROM characters WHERE guid = '%u'", guid);
...
void Player::UpdatePvPState(bool onlyFFA)
{
    ...
    if (!pvpInfo.IsInNoPvPArea && !pvpInfo.IsInHighSecZone && !IsGameMaster()
        && (pvpInfo.IsInFFAPvPArea || sWorld->IsFFAPvPRealm() || InBattleground()))
    {
        if (!IsFFAPvP())
        {
            SetByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP);
