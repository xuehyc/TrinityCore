            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 13567:                                 // Dummy Trigger
                {
                    ...
                    switch (m_triggeredByAuraSpell->Id)
                    {
                        case 26467:                         // Persistent Shield
                            ...
                            break;
                        default:
                            TC_LOG_ERROR("sql.sql", "EffectDummy: Non-handled case for spell 13567 for triggered aura %u", m_triggeredByAuraSpell->Id);
                            break;
                    }
                    return;
                }
                ...
                case 42784:                                // Wrath of the Astromancer
                {
                    ...
                    for (std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                        if (ihit->effectMask & (1<<effIndex))
                        {
                            if (Unit* casttarget = ObjectAccessor::GetUnit((*unitTarget), ihit->targetGUID))
