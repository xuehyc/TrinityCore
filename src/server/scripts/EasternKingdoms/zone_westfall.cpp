...
enum eQuest26209
{
    ...
    SPELL_APPLY_QUEST_INVIS_ZONE_0 = 79488,
    SPELL_APPLY_QUEST_INVIS_ZONE_1 = 78284,
    SPELL_APPLY_QUEST_INVIS_ZONE_2 = 79343,
    SPELL_COSMETIC_SLEEP_ZZZ = 78677,
};

class npc_investigator_42309 : public CreatureScript
{
public:
    npc_investigator_42309() : CreatureScript("npc_investigator_42309") { }

    struct npc_investigator_42309AI : public ScriptedAI
    {
        npc_investigator_42309AI(Creature *c) : ScriptedAI(c) { }
    ...
    {
        return new npc_investigator_42309AI(creature);
    }
};
    ...
    struct npc_horatio_lane_42308AI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (Player* player = who->ToPlayer())
                if (player->HasAura(SPELL_APPLY_QUEST_INVIS_ZONE_0))
                    if (player->GetQuestStatus(QUEST_HEROS_CALL_WESTFALL1) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_HEROS_CALL_WESTFALL2) == QUEST_STATUS_INCOMPLETE)
...
public:
    ...
    struct npc_lues_old_houseAI : public ScriptedAI
    {
        ...
        {
            m_timer = 0; m_phase = 0; 
            m_shadowy = NULL; m_glubtok = NULL; m_player = NULL;
        }

        void IsSummonedBy(Unit* summoner) override
        {
            if (m_player = summoner->ToPlayer())
                if (m_glubtok = me->SummonCreature(NPC_GLUBTOK_THE_FOREMAN,-9846.5f,1395.5f,37.5534f,0.63f,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,85000))
                    if (m_shadowy = me->SummonCreature(NPC_SHADOWY_FIGURE, -9825.17f, 1409.41f, 36.654f, 3.66f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000))
                    {
                        m_phase = 1; m_timer = 2500;
                    }
        }
        ...
        void DoWork()
        {
            ...
            case 1:
                ...
                m_shadowy->SetWalk(true);
                m_shadowy->GetMotionMaster()->MovePoint(0, -9841.677f, 1400.042f, 37.129f);
                break;
            case 2:
                ...
                break;
            case 3:
                m_phase++; m_timer = 8500;
                m_shadowy->AI()->Talk(0);
                break;
            case 4:
                m_phase++; m_timer = 4000;
                m_glubtok->AI()->Talk(1);
                break;
            case 5:
                m_phase++; m_timer = 6500;
                m_shadowy->AI()->Talk(1);
                break;
            case 6:
                m_phase++; m_timer = 6500;
            ...
            case 12:
                ...
                break;
            case 13:
                m_phase++; m_timer = 2000;
                m_player->KilledMonsterCredit(NPC_GLUBTOK_THE_FOREMAN);
                m_player->CompleteQuest(QUEST_LIVIN_THE_LIFE);
                m_shadowy->DespawnOrUnsummon(1000);
                m_glubtok->DespawnOrUnsummon(1000);
                break;
            case 14:
                m_phase=0; m_timer = 0;
...
enum eQuest26232
{
    ...
    // NPC_INVESTIGATOR_42559 = 42559, // already declared
    // NPC_HORATIO_LANE_42558 = 42558, // already declared
    // NPC_FURLBROW_MURDER_INFO_004 = 42417, // already declared
    // SPELL_APPLY_QUEST_INVIS_ZONE_0 = 79488, // already declared
    // SPELL_APPLY_QUEST_INVIS_ZONE_1 = 78284, // already declared
    // SPELL_APPLY_QUEST_INVIS_ZONE_2 = 79343, // already declared
    ...
    struct npc_thugAI : public ScriptedAI
    {
        ...
        { 
            ...
            if (m_phase > 8)
            {
                ...
                m_player->RemoveAura(SPELL_APPLY_QUEST_INVIS_ZONE_0);
                m_player->RemoveAura(SPELL_APPLY_QUEST_INVIS_ZONE_1);
                m_player->AddAura(SPELL_APPLY_QUEST_INVIS_ZONE_2, m_player);
            ...
            if (Player* player = who->ToPlayer())
                if (m_phase == 0)
                    if (player->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS) == QUEST_STATUS_COMPLETE)
                        if (player->HasAura(SPELL_APPLY_QUEST_INVIS_ZONE_2))
...
class npc_farmer_saldean : public CreatureScript
{
public:
    npc_farmer_saldean() : CreatureScript("npc_farmer_saldean") { }

    bool OnQuestReward(Player* player, Creature* /*creature*/, Quest const* quest, uint32 /*opt*/) 
    { 
        if (quest->GetQuestId()==26236)
            if (player->HasAura(SPELL_APPLY_QUEST_INVIS_ZONE_2))
            {
                player->RemoveAura(SPELL_APPLY_QUEST_INVIS_ZONE_2);
                return true;
            }
