...
void Log::write(std::unique_ptr<LogMessage>&& msg) const
{
    ...
    if (_ioContext)
    {
        ...
        Server::Asio::post(*_ioContext, Server::Asio::bind_executor(*_strand, [logOperation]() { logOperation->call(); }));
    }
    else
...
std::string Log::GetTimestampStr()
{
    ...
    try
    {
        return Server::StringFormat("%04d-%02d-%02d_%02d-%02d-%02d",
...
void Log::Initialize(Server::Asio::IoContext* ioContext)
{
    if (ioContext)
    {
        _ioContext = ioContext;
        _strand = new Server::Asio::Strand(*ioContext);
    }
