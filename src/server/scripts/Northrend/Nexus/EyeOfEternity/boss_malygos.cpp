        ...
        class spell_malygos_arcane_storm_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (GetSpellInfo()->Id == SPELL_ARCANE_STORM_P_III)
                {
                    ...
                    Server::Containers::RandomResize(targets, check, (malygos->GetMap()->GetDifficultyID() == DIFFICULTY_10_N ? 4 : 10));
                }
                else
                    Server::Containers::RandomResize(targets, (malygos->GetMap()->GetDifficultyID() == DIFFICULTY_10_N ? 4 : 10));
            }
            ...
            void FilterMeleeHoverDiskPassangers(std::list<WorldObject*>& targets)
            {
                ...
                if (targets.size() > 1)
                    if (malygos && !malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID).IsEmpty())
                        targets.remove_if(Server::ObjectGUIDCheck(malygos->AI()->GetGUID(DATA_LAST_TARGET_BARRAGE_GUID)));
                ...
                if (!targets.empty())
                    targets.remove_if(Server::UnitAuraCheck(true, SPELL_ARCANE_BARRAGE_DAMAGE));
    ...
    public:
        ...
        class spell_malygos_surge_of_power_warning_selector_25_SpellScript : public SpellScript
        {
            ...
            void SendThreeTargets(std::list<WorldObject*>& targets)
            {
                ...
                Server::Containers::RandomResize(selectedTargets, 3);
