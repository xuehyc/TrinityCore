    ...
    LOG_INFO("server.loading", "Loading Game Event Creature Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("sql.sql", "`game_event_creature` contains creature (GUID: " UI64FMTD ") not found in `creature` table.", guid);
                    continue;
                }
    ...
    LOG_INFO("server.loading", "Loading Game Event GO Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("sql.sql", "`game_event_gameobject` contains gameobject (GUID: " UI64FMTD ") not found in `gameobject` table.", guid);
                    continue;
                }
        ...
        else
        {
            ...
            do
            {
                ...
                if (newModelEquipSet.equipment_id > 0)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql", "Table `game_event_model_equip` contains creature (Guid: " UI64FMTD ", entry: %u) with equipment_id %u not found in table `creature_equip_template`. Setting entry to no equipment.",
