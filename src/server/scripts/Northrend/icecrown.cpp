...
-- Quest At The Enemy's Gates IS NOT WORKIN, WE NEED SNIF OF BLIZARD TO KNOW WHAT AURA USE FOR PLAYERS IN THE OTHER PHASE THEN SPAWN NPCS AND THE VILLA OF THIS QUEST.
...
public:
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature /*pCreature*/, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                pPlayer->CastSpell(pPlayer,SPELL_CREDIT_VALIS,true); // Cast the spell of Credit Quest (valid target)
                pPlayer->CLOSE_GOSSIP_MENU(); // Closes the window of the client-side Gossip
            break;
            case GOSSIP_ACTION_INFO_DEF+2:
                // Tell a  bla bla
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_HELLO_VALIS_1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
## SELECT entry,title FROM quest_template WHERE title LIKE 'The Grand Melee%'
######*/
...
/*######
## npc_captive_aspirant
######*/
class npc_captive_aspirant : public CreatureScript
{
public:
    enum GameObjects
    {
        GO_BLACK_CAGE = 195310
    };

    npc_captive_aspirant() : CreatureScript("npc_captive_aspirant") { }

    struct npc_captive_aspirantAI : public ScriptedAI
    {
        enum GameObjects
        {
            GO_GJALERBRON_CAGE = 186501,
            GO_LARGE_GJALERBRON_CAGE = 186490,
        };

        npc_captive_aspirantAI(Creature *pCreature) : ScriptedAI(pCreature) { }

        void Reset()
        {
            me->SetReactState(REACT_PASSIVE);

            if (GameObject* pGO = me->FindNearestGameObject(GO_BLACK_CAGE,5.0f))
            {
    ...
    CreatureAI *GetAI(Creature *creature) const
    {
        return new npc_captive_aspirantAI(creature);
    }
};
