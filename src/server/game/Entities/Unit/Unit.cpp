                        ...
                        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        {
                            if (i->getSource())
                            {
                                if (i->getSource()->IsInRaidWith(creditedPlayer))
                                {
                                    ...
                                    CharacterDatabase.PExecute("INSERT INTO castle_log VALUES (%u,'%s',1,UNIX_TIMESTAMP(),%u,%u,%u,'%s')", i->getSource()->GetGUIDLow(), player_name.c_str(), creature->GetEntry(), creature->GetCreatureTemplate()->Entry, instanceMap->GetDifficulty(), creature_name.c_str());
                                }
                            }
                        }
                    }
