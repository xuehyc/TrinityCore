...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    do
    {
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Creature (Entry: %u) has temporary aura (spell %lu) in `auras` field in `creature_template_addon`.", entry, atoul(*itr));
                continue;
            }
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    do
    {
        ...
        for (Tokenizer::const_iterator itr = tokens.begin(); itr != tokens.end(); ++itr)
        {
            ...
            if (AdditionalSpellInfo->GetDuration() > 0)
            {
                LOG_ERROR("sql.sql", "Creature (GUID: %u) has temporary aura (spell %lu) in `auras` field in `creature_addon`.", guid, atoul(*itr));
                continue;
            }
