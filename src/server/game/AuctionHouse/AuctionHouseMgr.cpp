...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    do
    {
        ...
        if (!proto)
        {
            LOG_ERROR("misc", "AuctionHouseMgr::LoadAuctionItems: Unknown item (GUID: %u item entry: #%u) in auction, skipped.", item_guid, itemEntry);
            continue;
        }
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data, Item* sourceItem) const
{
    ...
    if (!item)
    {
        LOG_ERROR("misc", "AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    {
        LOG_ERROR("misc", "Auction %u has invalid house id %u", Id, houseId);
        return false;
    }
    ...
    {
        LOG_ERROR("misc", "Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
...
std::string AuctionEntry::BuildAuctionMailSubject(MailAuctionAnswers response) const
{
    ...
    return Warhead::StringFormat("%u:%d:%u:%u:%u", itemEntry, item ? item->GetItemRandomPropertyId() : 0, response, Id, itemCount);
}

std::string AuctionEntry::BuildAuctionWonMailBody(ObjectGuid guid, uint32 bid, uint32 buyout)
{
    return Warhead::StringFormat("%llX:%u:%u", guid.GetRawValue(), bid, buyout);
}
...
{
    return Warhead::StringFormat("%llX:%u:%u:%u:%u", guid.GetRawValue(), bid, buyout, deposit, consignment);
}
...
{
    return Warhead::StringFormat("%llX:%u:%u:%u:%u:%u:%u", guid.GetRawValue(), bid, buyout, deposit, consignment, moneyDelay, eta);
}
