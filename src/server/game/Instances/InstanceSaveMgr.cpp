...
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, bool canReset, bool load)
{
    ...
    {
        LOG_ERROR("misc", "InstanceSaveManager::AddInstanceSave: mapid = %d, instanceid = %d, wrong dificalty %u!", mapId, instanceId, static_cast<uint32>(difficulty));
        return nullptr;
    }
...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    uint32 resetHour = CONF_GET_INT("Instance.ResetTimeHour");
    if (QueryResult result = CharacterDatabase.Query("SELECT mapid, difficulty, resettime FROM instance_reset"))
    {
        do
        {
            ...
            if (!mapDiff)
            {
                LOG_ERROR("misc", "InstanceSaveManager::LoadResetTimes: invalid mapid(%u)/difficulty(%u) pair in instance_reset!", mapid, static_cast<uint32>(difficulty));
...
time_t InstanceSaveManager::GetSubsequentResetTime(uint32 mapid, Difficulty difficulty, time_t resetTime) const
{
    ...
    time_t resetHour = CONF_GET_INT("Instance.ResetTimeHour");
    time_t period = uint32(((mapDiff->resetTime * CONF_GET_FLOAT("Rate.InstanceResetTime")) / DAY) * DAY);
    if (period < DAY)
        period = DAY;
...
void InstanceSaveManager::_ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime)
{
    ...
    if (!mapEntry->Instanceable())
        return;
    LOG_DEBUG("misc", "InstanceSaveManager::ResetOrWarnAll: Processing map %s (%u) on difficulty %u (warn? %u)", mapEntry->MapName[0], mapid, static_cast<uint32>(difficulty), warn);
