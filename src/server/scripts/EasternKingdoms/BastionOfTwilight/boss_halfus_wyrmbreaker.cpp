...
public:
    ...
    struct npc_halfus_dragon_prisonerAI : public ScriptedAI
    {
        ...
        {
            if (instance)
                if (Creature* halfus = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetData64(DATA_HALFUS))))
                ...
                case SPELL_FREE_DRAGON:
                    {
                        if (!instance)
                            return;
                        Creature* halfus = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetData64(DATA_HALFUS)));
                        if (!halfus)
                            return;
