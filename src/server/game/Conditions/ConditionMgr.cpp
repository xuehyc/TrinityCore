...
bool Condition::Meets(ConditionSourceInfo& sourceInfo) const
{
    ...
    switch (ConditionType)
    {
        ...
        {
            if (Unit* unit = object->ToUnit())
                condMeets = Server::RaceMask<uint64>{ ConditionValue1 }.HasRace(unit->getRace());
            break;
        }
...
bool ConditionMgr::IsObjectMeetToConditionList(ConditionSourceInfo& sourceInfo, ConditionContainer const& conditions) const
{
    ...
    {
        LOG_DEBUG("condition", "ConditionMgr::IsPlayerMeetToConditionList %s val1: %u", condition->ToString().c_str(), condition->ConditionValue1);
...
bool ConditionMgr::IsObjectMeetingSmartEventConditions(int64 entryOrGuid, uint32 eventId, uint32 sourceType, Unit* unit, WorldObject* baseObject) const
{
    ...
    if (itr != SmartEventConditionStore.end())
    {
        ...
        if (i != itr->second.end())
        {
            LOG_DEBUG("condition", "GetConditionsForSmartEvent: found conditions for Smart Event entry or guid " SI64FMTD " eventId %u", entryOrGuid, eventId);
...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    if (isReload)
    {
        LOG_INFO("misc", "Reseting Loot Conditions...");
        ...
        LOG_INFO("misc", "Re-Loading `gossip_menu` Table for Conditions!");
        sObjectMgr->LoadGossipMenu();

        LOG_INFO("misc", "Re-Loading `gossip_menu_option` Table for Conditions!");
...
bool ConditionMgr::addToPhases(Condition* cond) const
{
    ...
    LOG_ERROR("sql.sql", "%s Area %u does not have phase %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::isSourceTypeValid(Condition* cond) const
{
    switch (cond->SourceType)
    {
        case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceType, SourceEntry in `condition` table, item does not exist, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                LOG_ERROR("sql.sql", "SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_, __CHAIN__ for effect %u, SourceGroup needs correction, ignoring.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
        ...
        case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceEntry specifies non-existing quest, skipped.", cond->ToString().c_str());
                return false;
            }
            break;
        case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
        ...
        case CONDITION_SOURCE_TYPE_NPC_VENDOR:
        {
            ...
            if (!itemTemplate)
            {
                LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, item does not exist, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        }
        case CONDITION_SOURCE_TYPE_TERRAIN_SWAP:
        {
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in Map.dbc, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        }
        case CONDITION_SOURCE_TYPE_PHASE:
        {
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in AreaTable.dbc, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        }
        ...
        case CONDITION_SOURCE_TYPE_GRAVEYARD:
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in WorldSafeLocs.db2, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        default:
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_OBJECT_ENTRY_GUID_LEGACY:
            ...
            cond->ConditionValue1 = Server::Legacy::ConvertLegacyTypeID(Server::Legacy::TypeID(cond->ConditionValue1));
            /* fallthrough */
        case CONDITION_OBJECT_ENTRY_GUID:
        {
        ...
        case CONDITION_TYPE_MASK_LEGACY:
            ...
            cond->ConditionValue1 = Server::Legacy::ConvertLegacyTypeMask(cond->ConditionValue1);
            /* fallthrough */
        case CONDITION_TYPE_MASK:
        {
        ...
        case CONDITION_PHASEID:
        {
            ...
            {
                LOG_ERROR("sql.sql", "%s has nonexistent phaseid in value1 (%u), skipped", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        {
            LOG_ERROR("sql.sql", "%s using deprecated condition type CONDITION_SPAWNMASK.", cond->ToString(true).c_str());
            return false;
        }
        case CONDITION_UNIT_STATE:
        {
        ...
        case CONDITION_REALM_ACHIEVEMENT:
        {
            ...
            if (!achievement)
            {
                LOG_ERROR("sql.sql", "%s has non existing realm first achivement id (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        case CONDITION_QUEST_OBJECTIVE_COMPLETE:
        {
            ...
            if (!obj)
            {
                LOG_ERROR("sql.sql", "%s points to non-existing quest objective (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_PET_TYPE:
            ...
            {
                LOG_ERROR("sql.sql", "%s has non-existing pet type %u, skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            break;
        case CONDITION_INSTANCE_INFO:
        ...
        case CONDITION_DIFFICULTY_ID:
            ...
            {
                LOG_ERROR("sql.sql", "%s has non existing difficulty in value1 (%u), skipped.", cond->ToString(true).c_str(), cond->ConditionValue1);
                return false;
            }
            break;
        default:
            LOG_ERROR("sql.sql", "%s Invalid ConditionType in `condition` table, ignoring.", cond->ToString().c_str());
            return false;
    }

    if (cond->ConditionTarget >= cond->GetMaxAvailableConditionTargets())
    {
        LOG_ERROR("sql.sql", "%s in `condition` table, has incorrect ConditionTarget set, ignoring.", cond->ToString(true).c_str());
        return false;
    }
