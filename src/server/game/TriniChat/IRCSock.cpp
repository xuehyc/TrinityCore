...
bool IRCClient::InitSock()
{
    ...
    {
        sLog->outError(LOG_FILTER_CHATSYS, "IRC Error: Winsock Initialization Error");
        return false;
    }
    #endif
    if ((sIRC.SOCKET = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == -1)
    {
        sLog->outError(LOG_FILTER_CHATSYS, "IRC Error: Socket Error");
        return false;
    }
    int on = 1;
    if ( setsockopt ( sIRC.SOCKET, SOL_SOCKET, SO_REUSEADDR, ( const char* ) &on, sizeof ( on ) ) == -1 )
    {
        sLog->outError(LOG_FILTER_CHATSYS, "IRC Error: Invalid Socket");
        return false;
    }
    #ifdef _WIN32
    u_long iMode = 0;
...
bool IRCClient::Connect(const char *cHost, int nPort)
{
    ...
    {
        sLog->outError(LOG_FILTER_CHATSYS, "IRCLIENT: Could not resolve host: %s", cHost);
        return false;
    }
    ...
    {
        sLog->outError(LOG_FILTER_CHATSYS, "IRCLIENT: Cannot connect to %s", cHost);
        return false;
    }
    //FD_ZERO(&sIRC.sfdset);
...
bool IRCClient::SendData(const char *data)
{
    if(sIRC.Connected)
    {
        ...
        {
            sLog->outError(LOG_FILTER_CHATSYS, "IRC Error: Socket Receieve ** \n");
            //Disconnect();
            return false;
        }
    }
...
//    wchar_t bufferdata;
    ...
    if ( nBytesRecv == -1 )
    {
        sLog->outError(LOG_FILTER_CHATSYS, "Connection lost.");
        sIRC.Connected = false;
    }
    else
    {
        if (-1 == nBytesRecv)
        {
            sLog->outError(LOG_FILTER_CHATSYS, "Error occurred while receiving from socket.");
        }
        else
        {
            std::string reply;
