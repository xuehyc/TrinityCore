        ...
        void OnChat(Player* player, uint32 type, uint32 lang, std::string& msg, Group* group) override
        {
            ...
            switch (type)
            {
                ...
                case CHAT_MSG_BATTLEGROUND:
                    if (lang != LANG_ADDON)
                        LOG_DEBUG("chat.log.bg", "Player %s tells battleground with leader %s: %s",
                            player->GetName().c_str(), group ? group->GetLeaderName() : "<unknown>", msg.c_str());
                    else
                        LOG_DEBUG("chat.log.addon.bg", "Player %s tells battleground with leader %s: %s",
                            player->GetName().c_str(), group ? group->GetLeaderName() : "<unknown>", msg.c_str());
                    break;

                case CHAT_MSG_BATTLEGROUND_LEADER:
                    LOG_DEBUG("chat.log.bg", "Leader player %s tells battleground: %s",
                        player->GetName().c_str(), msg.c_str());
                    break;
            }
        }
