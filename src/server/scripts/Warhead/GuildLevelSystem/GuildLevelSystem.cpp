...
void GuildLevelSystem::LoadBaseCriteria()
{
    ...
    do
    {
        ...
        _guildCriteriaBase.emplace(criteriaID, _data);

        auto const& _stages = _guildStages.find(stageID);
        if (_stages == _guildStages.end())
        {
            std::vector<uint32> __vector = { criteriaID };
            _guildStages.emplace(stageID, __vector);
        }
        else
            _stages->second.emplace_back(criteriaID);
...
void GuildLevelSystem::GetRewardsCriteria(Player* player, uint32 sender, uint32 action)
{
    uint32 criteriaID = action - GLS_GOSSIP_CRITERIA_ID;
    ...
    // Reward spells
    for (auto const& [lowGuid, member] : player->GetGuild()->GetAllMembers())
    {
        if (!member.IsOnline())
...
void GuildLevelSystem::GetAllLink(Player* player, Creature* creature, uint32 action)
{
    uint32 criteriaID = action - GLS_GOSSIP_CRITERIA_ID;
...
{
    _guildCriteriaProgress.emplace(guildID, new GuildCriteria(guildID, 1));
}
