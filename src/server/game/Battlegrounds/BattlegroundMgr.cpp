...
void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype, uint32 arenaFaction)
{
    ...
    *data << uint8(arenatype);
    LOG_DEBUG("network", "BattlegroundMgr::BuildBattlegroundStatusPacket: arenatype = %u for bg instanceID %u, TypeID %u.", arenatype, bg->GetClientInstanceID(), bg->GetTypeID());
    ...
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS:                                // status_in_progress
            ...
            *data << uint8(arenaFaction == ALLIANCE ? 1 : 0);   // arenafaction (0 for horde, 1 for alliance)
            break;
        default:
            LOG_ERROR("bg.battleground", "Unknown BG status!");
            break;
    }
}
...
void BattlegroundMgr::LoadBattlegroundTemplates()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 battlegrounds. DB table `battleground_template` is empty.");
        return;
    }
...
void BattlegroundMgr::InitAutomaticArenaPointDistribution()
{
    ...
    LOG_DEBUG("bg.battleground", "Initializing Automatic Arena Point Distribution");
    if (wstime < curtime)
    {
        m_NextAutoDistributionTime = curtime;           // reset will be called in the next update
        LOG_DEBUG("bg.battleground", "Battleground: Next arena point distribution time in the past, reseting it now.");
    }
    else
        m_NextAutoDistributionTime = wstime;
    LOG_DEBUG("bg.battleground", "Automatic Arena Point Distribution initialized.");
}
...
void BattlegroundMgr::CheckBattleMasters()
{
    ...
    for (auto const& creatureTemplatePair : ctc)
    {
        ...
        {
            LOG_ERROR("sql.sql", "Creature_Template Entry: %u has UNIT_NPC_FLAG_BATTLEMASTER, but no data in the `battlemaster_entry` table. Removing flag.", creatureTemplatePair.first);
