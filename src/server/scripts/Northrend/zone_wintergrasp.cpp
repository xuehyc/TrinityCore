            ...
            bool OnGossipHello(Player* player) override
            {
                ...
                for (uint8 i = 0; i < graveyard.size(); i++)
                    if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
                        AddGossipItemFor(player, GOSSIP_ICON_CHAT, player->GetSession()->GetWarheadString(((BfGraveyardWG*)graveyard[i])->GetTextId()), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + i);
    ...
    public:
        ...
        struct npc_wg_queueAI : public ScriptedAI
        {
            ...
            bool OnGossipHello(Player* player) override
            {
                ...
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, player->GetSession()->GetWarheadString(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
