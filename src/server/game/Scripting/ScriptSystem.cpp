...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    LOG_INFO("server.loading", "Loading Script Waypoints for " UI64FMTD " creature(s)...", entryCount);
    ...
    do
    {
        ...
        if (!info)
        {
            LOG_ERROR("sql.sql", "SystemMgr: DB table script_waypoint has waypoint for non-existant creature entry %u", entry);
            continue;
        }

        if (!info->ScriptID)
            LOG_ERROR("sql.sql", "SystemMgr: DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.", entry);
