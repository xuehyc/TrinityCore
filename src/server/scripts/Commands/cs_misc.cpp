...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = Server::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = Server::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
        ...
        char const* unknown = handler->GetServerString(LANG_UNKNOWN);
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetServerString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            if (!spellInfo)
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, owner == handler->GetSession()->GetPlayer() ? handler->GetServerString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, owner == handler->GetSession()->GetPlayer() ? handler->GetServerString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleKickPlayerCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string kickReasonStr = handler->GetServerString(LANG_NO_REASON);
        if (*args != '\0')
        {
...
#define SPELL_UNSTUCK_VISUAL 2683
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetServerString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetServerString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetServerString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetServerString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        LOG_DEBUG("misc", handler->GetServerString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        LOG_DEBUG("misc", handler->GetServerString(LANG_ADDITEMSET), itemSetId);
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName          = handler->GetServerString(LANG_ERROR);
        uint32 accId                  = 0;
        ObjectGuid::LowType lowguid   = targetGuid.GetCounter();
        std::string eMail             = handler->GetServerString(LANG_ERROR);
        std::string regMail           = handler->GetServerString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetServerString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetServerString(LANG_ERROR);
        uint32 failedLogins           = 0;
        uint32 latency                = 0;
        std::string OS                = handler->GetServerString(LANG_UNKNOWN);

        // Mute data print variables
        int64 muteTime                = -1;
        std::string muteReason        = handler->GetServerString(LANG_NO_REASON);
        std::string muteBy            = handler->GetServerString(LANG_UNKNOWN);

        // Ban data print variables
        int64 banTime                 = -1;
        std::string banType           = handler->GetServerString(LANG_UNKNOWN);
        std::string banReason         = handler->GetServerString(LANG_NO_REASON);
        std::string bannedBy          = handler->GetServerString(LANG_UNKNOWN);

        // Character data print variables
        uint8 raceid, classid           = 0; //RACE_NONE, CLASS_NONE
        std::string raceStr, classStr   = handler->GetServerString(LANG_UNKNOWN);
        uint8 gender                    = 0;
        LocaleConstant locale           = handler->GetSessionDbcLocale();
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetServerString(LANG_ERROR);
        uint64 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        uint32 mapId;
        uint32 areaId;
        std::string areaName    = handler->GetServerString(LANG_UNKNOWN);
        std::string zoneName    = handler->GetServerString(LANG_UNKNOWN);
        ...
        if (target)
        {
            ...
            alive             = target->IsAlive() ? handler->GetServerString(LANG_YES) : handler->GetServerString(LANG_NO);
        ...
        else
        {
            ...
            if (!health || playerFlags & PLAYER_FLAGS_GHOST)
                alive = handler->GetServerString(LANG_NO);
            else
                alive = handler->GetServerString(LANG_YES);
        }
        ...
        if (result)
        {
            ...
            {
                eMail     = handler->GetServerString(LANG_UNAUTHORIZED);
                regMail   = handler->GetServerString(LANG_UNAUTHORIZED);
                lastIp    = handler->GetServerString(LANG_UNAUTHORIZED);
                lastLogin = handler->GetServerString(LANG_UNAUTHORIZED);
            }
        ...
        if (!result2)
        {
            banType = handler->GetServerString(LANG_CHARACTER);
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetServerString(LANG_OFFLINE), nameLink.c_str(), targetGuid.ToString().c_str());
        ...
        if (banTime >= 0)
            handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - time(NULL), true).c_str() : handler->GetServerString(LANG_PERMANENTLY), bannedBy.c_str());
        ...
        handler->PSendSysMessage(LANG_PINFO_ACC_IP, lastIp.c_str(), locked ? handler->GetServerString(LANG_YES) : handler->GetServerString(LANG_NO));
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetServerString(LANG_CHARACTER_GENDER_MALE) : handler->GetServerString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
        ...
        if (target)
            handler->PSendSysMessage(LANG_PINFO_CHR_MAP, map->MapName->Str[handler->GetSessionDbcLocale()],
                (!zoneName.empty() ? zoneName.c_str() : handler->GetServerString(LANG_UNKNOWN)),
                (!areaName.empty() ? areaName.c_str() : handler->GetServerString(LANG_UNKNOWN)));
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        Server::WorldObjectWorker<Server::RespawnDo> worker(player, u_do);
    ...
    static bool HandleMuteCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string muteReasonStr = handler->GetServerString(LANG_NO_REASON);
        if (muteReason != NULL)
        ...
        if (handler->GetSession())
            ...
            muteBy = handler->GetServerString(LANG_CONSOLE);
