...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    switch (targetType.GetObjectType())
    {
        ...
        case TARGET_OBJECT_TYPE_CORPSE:
            ...
            {
                LOG_DEBUG("spells", "Spell::SelectImplicitNearbyTargets: OnObjectTargetSelect script hook for spell Id %u set object of wrong type, expected corpse, got %s, effect %u", m_spellInfo->Id, target->GetGUID().GetTypeName(), effMask);
                SendCastResult(SPELL_FAILED_BAD_IMPLICIT_TARGETS);
                finish(false);
                return;
            }
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
    {
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Warhead::Containers::RandomResize(targets, maxTargets);
            }
...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    std::list<WorldObject*> targets;
    Warhead::WorldObjectSpellTrajTargetCheck check(dist2d, &srcPos, m_caster, m_spellInfo, targetType.GetCheckType(), m_spellInfo->Effects[effIndex].ImplicitTargetConditions);
    ...
    SearchTargets<Warhead::WorldObjectListSearcher<Warhead::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, &srcPos, dist2d);
    if (targets.empty())
        return;
...
GameObject* Spell::SearchSpellFocus()
{
    ...
    Warhead::GameObjectFocusCheck check(m_caster, m_spellInfo->RequiresSpellFocus);
    Warhead::GameObjectSearcher<Warhead::GameObjectFocusCheck> searcher(m_caster, focus, check);
...
void Spell::finish(bool ok)
{
    ...
    if (unitCaster->GetTypeId() == TYPEID_UNIT && unitCaster->IsSummon())
    {
        ...
        {
            LOG_DEBUG("spells", "Statue %s is unsummoned in spell %d finish", unitCaster->GetGUID().ToString().c_str(), m_spellInfo->Id);
