...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    {
        LOG_GM(m_pPlayer->GetSession()->GetAccountId(),
...
void Guild::HandleSetRankInfo(WorldSession* session, uint8 rankId, std::string_view name, uint32 rights, uint32 moneyPerDay, std::array<GuildBankRightsAndSlots, GUILD_BANK_MAX_TABS> const& rightsAndSlots)
{
    ...
    else if (RankInfo* rankInfo = GetRankInfo(rankId))
    {
        ...
        LOG_DEBUG("guild", "Changed RankName to '%s', rights to 0x%08X", rankInfo->GetName().c_str(), rights);
    }
}
...
void Guild::HandleInviteMember(WorldSession* session, std::string_view name)
{
    ...
    LOG_DEBUG("guild", "Player %s invited %s to join his Guild", player->GetName().c_str(), pInvitee->GetName().c_str());
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    _BroadcastEvent(GE_BANK_MONEY_SET, ObjectGuid::Empty, Warhead::StringFormat("%016llX", m_bankMoney));
...
bool Guild::LoadMemberFromDB(Field* fields)
{
    ...
    if (!isNew)
    {
        LOG_ERROR("guild", "Tried to add %s to guild '%s'. Member already exists.", playerGuid.ToString().c_str(), m_name.c_str());
        return false;
    }
...
bool Guild::AddMember(CharacterDatabaseTransaction trans, ObjectGuid guid, uint8 rankId)
{
    ...
    if (!isNew)
    {
        LOG_ERROR("guild", "Tried to add %s to guild '%s'. Member already exists.", guid.ToString().c_str(), m_name.c_str());
        return false;
    }
...
void Guild::_CreateDefaultGuildRanks(CharacterDatabaseTransaction trans, LocaleConstant loc)
{
    ...
    _CreateRank(trans, sGameLocale->GetWarheadString(LANG_GUILD_MASTER,   loc), GR_RIGHT_ALL);
    _CreateRank(trans, sGameLocale->GetWarheadString(LANG_GUILD_OFFICER,  loc), GR_RIGHT_ALL);
    _CreateRank(trans, sGameLocale->GetWarheadString(LANG_GUILD_VETERAN,  loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(trans, sGameLocale->GetWarheadString(LANG_GUILD_MEMBER,   loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
    _CreateRank(trans, sGameLocale->GetWarheadString(LANG_GUILD_INITIATE, loc), GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, ObjectGuid guid, std::string_view param1, std::string_view param2, std::string_view param3) const
{
    ...
    LOG_DEBUG("guild", "SMSG_GUILD_EVENT [Broadcast] Event: %s (%u)", GetGuildEventString(guildEvent), guildEvent);
}
