...
void IRCClient::Handle_WoW_Channel(std::string Channel, Player *plr, int nAction)
{
    ...
    if(sIRC.Connected && (sIRC.BOTMASK & 1)!= 0)
    {
        if(Channel_Valid(Channel))
        {
            ...
            std::string query = "REPLACE INTO `irc_inchan` VALUES (%d,'%s','%s')";
            std::string lchan = "DELETE FROM `irc_inchan` WHERE `guid` = %d AND `channel` = '%s'";
            switch(nAction)
            {
                case CHANNEL_JOIN:
                    // Do not send any join / leave messages to IRC
                    ...
                    //WorldDatabase.PExecute(lchan.c_str(), plr->GetGUID(), Channel.c_str());
                    WorldDatabase.PExecute(query.c_str(), plr->GetGUID(), pname.c_str(), Channel.c_str());
                    break;
                case CHANNEL_LEAVE:
