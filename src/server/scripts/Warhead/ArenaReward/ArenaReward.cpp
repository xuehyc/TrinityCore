...
namespace
{
    ...
    void ParalysePlayer(Player* player)
    {
        if (CONF_GET_BOOL("ArenaReward.AntiFarm.Teleport.Enable"))
        {
            ...
            player->TeleportTo(WorldLocation(mapID, posX, posY, posZ, orientation), 0);
...
void ArenaReward::SendRewardArena(Battleground* bg, uint32 winner)
{
    ...
    auto SendItems = [&](uint32 itemID, uint32 itemCountWinner, uint32 itemCountLoser)
    {
        ...
        {
            auto winnerArenaTeam = sArenaTeamMgr->GetArenaTeamById(bg->GetArenaTeamIdForTeam(winner));
            auto loserArenaTeam = sArenaTeamMgr->GetArenaTeamById(bg->GetArenaTeamIdForTeam(bg->GetOtherTeam(winner)));
        ...
        else if (CONF_GET_BOOL("ArenaReward.Reward.Skirmish.Enable")) // !isRated
        {
            for (auto const& [guid, bgPlayer] : bg->GetPlayers())
            {
                ...
                bool isWinner = player->GetBGTeam() == winner;
    ...
    if (!IsPossibleFarm(bg, winner))
        SendItems(rewardItemID, rewardItemCountWinner, rewardItemCountLoser);
}

bool ArenaReward::IsPossibleFarm(Battleground* bg, uint32 winner)
{
    ...
    if (CheckIP(bg, winner))
        isFarmer = true;

    if (CheckEqipment(bg, winner))
        isFarmer = true;

    if (CheckHealth(bg, winner))
        isFarmer = true;
...
bool ArenaReward::CheckIP(Battleground* bg, uint32 winner)
{
    ...
    if (isRated)
    {
        ...
        for (auto const& member : winnerArenaTeam->GetMembers())
        {
            ...
            playerStore.emplace_back(player, player->GetSession()->GetRemoteAddress(), true);
        }
        ...
        for (auto const& member : loserArenaTeam->GetMembers())
        {
            ...
            playerStore.emplace_back(player, player->GetSession()->GetRemoteAddress(), false);
        }
    }
    else // !isRated
    {
...
bool ArenaReward::CheckEqipment(Battleground* bg, uint32 winner)
{
...
bool ArenaReward::CheckHealth(Battleground* bg, uint32 winner)
{
