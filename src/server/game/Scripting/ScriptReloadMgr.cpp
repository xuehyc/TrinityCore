...
static bool HasValidScriptModuleName(std::string const& name)
{
    ...
    static Server::regex const regex(
        Server::StringFormat("^%s[sS]cripts_[a-zA-Z0-9_]+\\.%s$",
            GetSharedLibraryPrefix(),
            GetSharedLibraryExtension()));

    return Server::regex_match(name, regex);
}
...
static int InvokeCMakeCommand(T&&... args)
{
    ...
    return Server::StartProcess(executable, {
        executable,
        std::forward<T>(args)...
    }, "scripts.hotswap");
}
...
template<typename... T>
static std::shared_ptr<Server::AsyncProcessResult> InvokeAsyncCMakeCommand(T&&... args)
{
    auto const executable = BuiltInConfig::GetCMakeCommand();
    return Server::StartAsyncProcess(executable, {
        executable,
        std::forward<T>(args)...
    }, "scripts.hotswap");
}
    ...
    : public ScriptReloadMgr
{
    ...
    class BuildJob
    {
        ...
        std::shared_ptr<Server::AsyncProcessResult> async_result_;
    ...
    public:
        ...
        std::shared_ptr<Server::AsyncProcessResult> const& GetProcess() const
        {
        ...
        void UpdateCurrentJob(BuildJobType type,
                              std::shared_ptr<Server::AsyncProcessResult> async_result)
        {
...
private:
    ...
    static fs::path CalculateTemporaryCachePath()
    {
        ...
        path /= Server::StringFormat("tc_script_cache_%s_%s",
    ...
    fs::path GenerateUniquePathForLibraryInCache(fs::path path)
    {
        ...
        cache_path /= Server::StringFormat("%s.%u%s",
    ...
    void DispatchRunningBuildJobs()
    {
        ...
        if (IsDebuggerBlockingRebuild())
        {
                ...
                (GetMSTimeDiffToNow(_last_time_user_informed) > 7500))
            {
                ...
                LOG_INFO("scripts.hotswap", "Your attached debugger is blocking the MobiusCore "
...
{
    static Server::regex const regex("^\\.(h|hpp|c|cc|cpp)$");
    return Server::regex_match(path.extension().generic_string(), regex);
}
