...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Creature>(uint64 db_guid) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<GameObject>(uint64 db_guid) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Pool>(uint64 sub_pool_id) const
{
...
template<>
GAME_API bool ActivePoolData::IsActiveObject<Quest>(uint64 quest_id) const
{
...
void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint64 triggerFrom)
{
    ...
    if (limit > newQuests.size() && !currentQuests.empty())
    {
        do
        {
            uint32 questId = Server::Containers::SelectRandomContainerElement(currentQuests);
    ...
    do
    {
        uint32 questId = Server::Containers::SelectRandomContainerElement(newQuests);
        spawns.ActivateObject<Quest>(questId, poolId);
        PoolObject tempObj(questId, 0.0f);
        Spawn1Object(&tempObj);
    ...
    LOG_INFO("server.loading", "Loading Creatures Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("sql.sql", "`pool_creature` has a non existing creature spawn (GUID: " UI64FMTD ") defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql", "`pool_creature` has an invalid chance (%f) for creature guid (" UI64FMTD ") in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
                PoolTemplateData* pPoolTemplate = &mPoolTemplate[pool_id];
    ...
    LOG_INFO("server.loading", "Loading Gameobject Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if (!data)
                {
                    LOG_ERROR("sql.sql", "`pool_gameobject` has a non existing gameobject spawn (GUID: " UI64FMTD ") defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql", "`pool_gameobject` has a not lootable gameobject spawn (GUID: " UI64FMTD ", type: %u) defined for pool id (%u), skipped.", guid, goinfo->type, pool_id);
                    continue;
                }
                ...
                if (chance < 0 || chance > 100)
                {
                    LOG_ERROR("sql.sql", "`pool_gameobject` has an invalid chance (%f) for gameobject guid (" UI64FMTD ") in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
