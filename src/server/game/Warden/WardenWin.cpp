...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    {
        LOG_WARN("warden", "%s failed hash reply. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
        return;
    }
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    {
        buff.rpos(buff.wpos());
        LOG_WARN("warden", "%s failed checksum. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            LOG_WARN("warden", "%s failed timing check. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
            return;
        }
    ...
    if (checkFailed > 0)
    {
        ...
        LOG_WARN("warden", "%s failed Warden check %u. Action: %s", _session->GetPlayerInfo().c_str(), checkFailed, Penalty(check).c_str());
    }
