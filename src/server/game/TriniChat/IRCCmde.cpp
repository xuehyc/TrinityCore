...
void IRCCmd::Handle_Login(_CDATA *CD)
{
    ...
    if(isbanned == "NOTBANNED")
    {
        if(!IsLoggedIn(CD->USER))
        {
            ...
            {
                QueryResult result = LoginDatabase.PQuery("SELECT aa.gmlevel FROM account a, account_access aa WHERE aa.id = a.id AND a.username='%s' AND a.sha_pass_hash=SHA1(CONCAT(UPPER(a.username),':',UPPER('%s')));", _PARAMS[0].c_str(), _PARAMS[1].c_str());
                if (result)
                {
...
void IRCCmd::Ban_Player(_CDATA *CD)
{
    ...
    if(_PARAMS[1] == "unban")
    {
        ...
        else
        {
            ...
            {
                Field *fields = result->Fetch();
                std::string id = fields[0].GetCString();
    ...
    if(plr)
    {
        ...
        if(_PARAMS[1] == "quest")
        {
            ...
            if(_PARAMS[2] == "add")
            {
                ...
                ItemTemplateContainer::const_iterator result = find_if(itc->begin(), itc->end(), Finder<uint32, ItemTemplate>(qId, &ItemTemplate::StartQuest));
            ...
            if(_PARAMS[2] == "remove")
            {
                ...
                plr->getQuestStatusMap()[qId].Status = QUEST_STATUS_NONE;
            ...
            if(_PARAMS[2] == "complete")
            {
                ...
                else
                {
                    for(uint8 x = 0; x < QUEST_OBJECTIVES_COUNT; ++x)
                    {
                        ...
                        InventoryResult msg = plr->CanStoreNewItem( NULL_BAG, NULL_SLOT, dest, id, count-curItemCount );
                        if( msg == EQUIP_ERR_OK )
                        {
    ...
    if(result)
    {
        if(IsLoggedIn(CD->USER))
        {
            ...
            else
            {
                ...
                {
                    Field *fields = result->Fetch();
                    if(fields[1].GetUInt32() > (GetLevel(CD->USER) < 0 ? 0 : (uint32)GetLevel(CD->USER)))
                    {
                        Send_IRCA(CD->USER, "You Do Not Have Access To That Command, So No Help Is Available.", true, CD->TYPE.c_str());
                        return;
                    }
                    std::string cmdhlp = fields[0].GetCString();
...
void IRCCmd::Inchan_Server(_CDATA *CD)
{
    ...
    {
        Field *fields = result->Fetch();
        std::string output = "\002Players In The [ ";
        output += (*fields[2].GetCString()) + " ] Channel:\017 ";

        for (uint64 i=0; i < result->GetRowCount(); i++)
        {
            output += (*fields[1].GetCString()) + ", ";
            result->NextRow();
        }
    ...
    if(ValidParams(CD->PARAMS, 1))
    {
        ...
        if (Player *plr = GetPlayer(_PARAMS[0]))
        {
            ...
            if(_PARAMS[1] == "release")
            {
                ...
                QueryResult result = CharacterDatabase.PQuery( "SELECT `map`, `position_x`, `position_y`, `position_z` FROM `character_homebind` WHERE `guid` = '%u'", plr->GetGUID() );
                if(result)
                {
                    Field *fields = result->Fetch();
...
void IRCCmd::Lookup_Player(_CDATA *CD)
{
    ...
    if(_PARAMS[0] == "acct")
    {
        ...
        if(acctid > 0)
        {
            ...
            QueryResult result = LoginDatabase.PQuery("SELECT a.id, a.username, aa.gmlevel, a.last_ip, (SELECT banreason FROM account_banned WHERE id = %d LIMIT 1) as banned, (SELECT banreason FROM ip_banned WHERE ip = a.last_ip) as bannedip, DATE_FORMAT(a.last_login, '%s') FROM account a, account_access aa WHERE aa.id = %d AND a.id = %d", acctid, DateTime.c_str(), acctid, acctid);
            if(result)
            {
                Field *fields = result->Fetch();

                uint32 id = fields[0].GetUInt32();
                std::string usrname = fields[1].GetCString();
                uint32 gm = fields[2].GetUInt32();
                std::string lastip = fields[3].GetCString();
                std::string banreason = fields[4].GetCString();
                std::string banreasonip = fields[5].GetCString();
                std::string lastlogin = fields[6].GetCString();
                ...
                if(chars)
                {
                    ...
                    totaccttime = SecToDay(fields[2].GetCString());
                    for (uint64 i=0; i < chars->GetRowCount(); i++)
                    {
                        std::string guid = fields[0].GetCString();
                        std::string charname = fields[1].GetCString();
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string acctid = fields[0].GetCString();
                    std::string acctname = fields[1].GetCString();
    ...
    if(_PARAMS[0] == "char")
    {
        uint64 plguid = atoi(_PARAMS[1].c_str());
        ...
        if(plguid > 0)
        {
            ...
            {
                Field *fields = result->Fetch();
                std::string pguid = fields[0].GetCString();
                std::string pacct = fields[1].GetCString();
                std::string pname = fields[2].GetCString();
        ...
        else
        {
            ...
            if(result)
            {
                ...
                {
                    std::string guid = fields[0].GetCString();
                    std::string account = fields[1].GetCString();
    ...
    if(_PARAMS[0] == "creature")
    {
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string CreatureID = fields[0].GetCString();
    ...
    if(_PARAMS[0] == "go")
    {
        ...
        if(atoi(gobject.c_str()) > 0)
        {
            ...
            if(result)
            {
                ...
                std::string name = fields[3].GetCString();
                uint32 faction = fields[4].GetUInt32();
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string GOID = fields[0].GetCString();
                    std::string GoName = fields[1].GetCString();
    ...
    if(_PARAMS[0] == "item")
    {
        ...
        if(atoi(item.c_str()) > 0)
        {
            ...
            if(result)
            {
                ...
                std::string ItmName = fields[1].GetCString();
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string ItemID = fields[0].GetCString();
                    std::string ItemName = fields[1].GetCString();
    ...
    if(_PARAMS[0] == "quest")
    {
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string QuestID = fields[0].GetCString();
                    std::string QuestName = fields[1].GetCString();
    ...
    if(_PARAMS[0] == "tele")
    {
        ...
        if(atoi(tele.c_str()) > 0)
        {
            ...
            if(result)
            {
                ...
                std::string telname = fields[6].GetCString();
        ...
        else
        {
            ...
            if(result)
            {
                ...
                for (uint64 i=0; i < result->GetRowCount(); i++)
                {
                    std::string TeleID = fields[0].GetCString();
                    std::string TeleName = fields[1].GetCString();
...
void IRCCmd::Level_Player(_CDATA *CD)
{
    ...
    } else
    {
        ...
        int32 i_oldlvl = chr ? chr->getLevel() : 0;
        if(chr)
        {
        ...
        {
            Send_IRCA(CD->USER, MakeMsg("\0034[ERROR] : Spieler ist nicht auffindbar"), true, "ERROR");
        }
    }
...
void IRCCmd::Money_Player(_CDATA *CD)
{
    ...
    else
    {
        ...
        else
        {
            ...
            std::string sqlquery = "SELECT `money` AS `gold` FROM `characters` WHERE `name` = '"+player+"';";
            if (QueryResult result = CharacterDatabase.Query(sqlquery.c_str()))
...
void IRCCmd::Mute_Player(_CDATA *CD)
{
    ...
    if(guid)
    {
        ...
        else
        {
            ...
            LoginDatabase.PExecute("UPDATE `account` SET `mutetime` = %u WHERE `id` = '%u'",uint64(mutetime), account_id );
...
void IRCCmd::Sysmsg_Server(_CDATA *CD)
{
    ...
    if(_PARAMS[0] == "a")
    {
        ...
        sWorld->SendWorldText(3,str.c_str());
    ...
    else if (_PARAMS[0] == "list")
    {
        ...
        if(result)
        {
            ...
            {
                std::string id = fields[0].GetCString();
                std::string message = fields[1].GetCString();
                std::string addedby = fields[2].GetCString();
...
void IRCCmd::Tele_Player(_CDATA *CD)
{
    ...
    if(_PARAMS[1] == "l" || _PARAMS[1].size() > 2)
    {
        ...
        else
        {
            ...
            if (result)
            {
                ...
                {
                    Field *fields = result->Fetch();
                    telename.append(fields[0].GetCString());
                    result->NextRow();
                    telename.append(" <> ");
                }
    ...
    {
        std::string* _PARAMSA = getArray(_PARAMS[2], 4);
        pX = (float)atof(_PARAMSA[1].c_str());
        pY = (float)atof(_PARAMSA[2].c_str());
        pZ = (float)atof(_PARAMSA[3].c_str());
    ...
    if(_PARAMS[0] == "accttime")
    {
        ...
        if(result)
        {
            ...
            for (uint64 i=0; i < result->GetRowCount(); i++)
            {
                ...
                std::string Time = SecToDay(fields[2].GetCString());
                uint64 rank = i+1;
    ...
    if(_PARAMS[0] == "chartime")
    {
        ...
        if(result)
        {
            ...
            for (uint64 i=0; i < result->GetRowCount(); i++)
            {
                ...
                std::string Time = SecToDay(fields[1].GetCString());
                uint64 rank = i+1;
