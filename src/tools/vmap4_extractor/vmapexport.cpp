...
bool ExtractSingleWmo(std::string& fname)
{
    ...
    if (froot.nGroups !=0)
    {
        for (uint32 i = 0; i < froot.nGroups; ++i)
        {
            ...
            WMOGroup fgroup(Warhead::StringFormat("%s_%03u.wmo", temp, i));
...
bool fillArchiveNameVector(std::vector<std::string>& pArchiveNames)
{
    ...
    if (!scan_patches(Warhead::StringFormat("%spatch", input_path).c_str(), pArchiveNames))
        return(false);
    ...
    {
        printf("Locale: %s\n", locale.c_str());
        if(scan_patches(Warhead::StringFormat("%s%s/patch-%s", input_path, locale.c_str(), locale.c_str()).c_str(), pArchiveNames))
            foundOne = true;
    }
...
{
    Warhead::Banner::Show("VMAP data extractor", [](char const* text) { printf("%s\n", text); }, nullptr);
