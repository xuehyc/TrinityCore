...
void ObjectMgr::LoadGossipMenuItemsLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded %u gossip_menu_option locale strings in %u ms", uint32(_gossipMenuItemsLocaleStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " creature definitions in %u ms", _creatureTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
    ...
    do
    {
        ...
        for (std::string_view aura : Warhead::Tokenize(fields[7].GetStringView(), ' ', false))
        {
            ...
            if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE))
                LOG_ERROR("sql.sql", "Creature (Entry: %u) has SPELL_AURA_CONTROL_VEHICLE aura %u defined in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);

            if (std::find(creatureAddon.auras.begin(), creatureAddon.auras.end(), spellInfo->Id) != creatureAddon.auras.end())
            {
                LOG_ERROR("sql.sql", "Creature (Entry: %u) has duplicate aura (spell %u) in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);
                continue;
            }

            if (spellInfo->GetDuration() > 0)
            {
                LOG_ERROR("sql.sql", "Creature (Entry: %u) has temporary aura (spell %u) in `auras` field in `creature_template_addon`.", entry, spellInfo->Id);
                continue;
            }
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (cInfo->GossipMenuId && !(cInfo->npcflag & UNIT_NPC_FLAG_GOSSIP))
        LOG_INFO("sql.sql", "Creature (Entry: %u) has assigned gossip menu %u, but npcflag does not include UNIT_NPC_FLAG_GOSSIP.", cInfo->Entry, cInfo->GossipMenuId);
    else if (!cInfo->GossipMenuId && cInfo->npcflag & UNIT_NPC_FLAG_GOSSIP)
        LOG_INFO("sql.sql", "Creature (Entry: %u) has npcflag UNIT_NPC_FLAG_GOSSIP, but gossip menu is unassigned.", cInfo->Entry);
}
...
void ObjectMgr::CheckCreatureMovement(char const* table, uint64 id, CreatureMovementData& creatureMovement)
{
    ...
    {
        LOG_ERROR("sql.sql", "`%s`.`Chase` wrong value (%u) for Id " UI64FMTD ", setting to Run.",
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    do
    {
        ...
        for (std::string_view aura : Warhead::Tokenize(fields[7].GetStringView(), ' ', false))
        {
            ...
            if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE))
                LOG_ERROR("sql.sql", "Creature (GUID: %u) has SPELL_AURA_CONTROL_VEHICLE aura %u defined in `auras` field in `creature_addon`.", guid, spellInfo->Id);

            if (std::find(creatureAddon.auras.begin(), creatureAddon.auras.end(), spellInfo->Id) != creatureAddon.auras.end())
            {
                LOG_ERROR("sql.sql", "Creature (GUID: %u) has duplicate aura (spell %u) in `auras` field in `creature_addon`.", guid, spellInfo->Id);
                continue;
            }

            if (spellInfo->GetDuration() > 0)
            {
                LOG_ERROR("sql.sql", "Creature (GUID: %u) has temporary aura (spell %u) in `auras` field in `creature_addon`.", guid, spellInfo->Id);
                continue;
            }
...
void ObjectMgr::LoadPlayerTotemModels()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Wrong TotemSlot %u in `player_totem_model` table, skipped.", totemSlot);
            continue;
        }
        ...
        if (!raceEntry)
        {
            LOG_ERROR("sql.sql", "Race %u defined in `player_totem_model` does not exists, skipped.", uint32(race));
            continue;
        }
        ...
        if (!displayEntry)
        {
            LOG_ERROR("sql.sql", "TotemSlot: %u defined in `player_totem_model` has non-existing model (%u), skipped.", totemSlot, displayId);
            continue;
        }
    ...
    LOG_INFO("server.loading", ">> Loaded %u player totem model records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
void ObjectMgr::LoadLinkedRespawn()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 linked respawns. DB table `linked_respawn` is empty.");
        return;
    }
...
void ObjectMgr::LoadCreatures()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 creatures. DB table `creature` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `creature` has creature (GUID: %u Entry: %u) with `wander_distance`< 0, set to 0.", guid, data.id);
        ...
        else if (data.movementType == RANDOM_MOTION_TYPE)
        {
            if (data.wander_distance == 0.0f)
            {
                LOG_ERROR("sql.sql", "Table `creature` has creature (GUID: %u Entry: %u) with `MovementType`=1 (random movement) but with `wander_distance`=0, replace by idle movement type (0).", guid, data.id);
                data.movementType = IDLE_MOTION_TYPE;
            }
        }
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.wander_distance != 0.0f)
            {
                LOG_ERROR("sql.sql", "Table `creature` has creature (GUID: %u Entry: %u) with `MovementType`=0 (idle) have `wander_distance`<>0, set to 0.", guid, data.id);
                data.wander_distance = 0.0f;
            }
        }
        ...
        if (data.phaseMask == 0)
        {
            LOG_ERROR("sql.sql", "Table `creature` has creature (GUID: %u Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
...
void ObjectMgr::AddCreatureToGrid(ObjectGuid::LowType guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = Warhead::ComputeCellCoord(data->spawnPoint.GetPositionX(), data->spawnPoint.GetPositionY());
...
void ObjectMgr::LoadGameObjects()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 gameobjects. DB table `gameobject` is empty.");
        return;
    }
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    do
    {
        ...
        if (dbcitem)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct class %u, must be %u .", entry, itemTemplate.Class, dbcitem->ClassID);
            ...
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct SoundOverrideSubclass (%i), must be %i .", entry, itemTemplate.SoundOverrideSubclass, dbcitem->SoundOverrideSubclassID);
                if (enforceDBCAttributes)
                    itemTemplate.SoundOverrideSubclass = dbcitem->SoundOverrideSubclassID;
            }
            if (itemTemplate.Material != dbcitem->Material)
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct material (%i), must be %i .", entry, itemTemplate.Material, dbcitem->Material);
                if (enforceDBCAttributes)
                    itemTemplate.Material = dbcitem->Material;
            }
            if (itemTemplate.InventoryType != dbcitem->InventoryType)
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct inventory type (%u), must be %u .", entry, itemTemplate.InventoryType, dbcitem->InventoryType);
                if (enforceDBCAttributes)
                    itemTemplate.InventoryType = dbcitem->InventoryType;
            }
            if (itemTemplate.DisplayInfoID != dbcitem->DisplayInfoID)
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct display id (%u), must be %u .", entry, itemTemplate.DisplayInfoID, dbcitem->DisplayInfoID);
                if (enforceDBCAttributes)
                    itemTemplate.DisplayInfoID = dbcitem->DisplayInfoID;
            }
            if (itemTemplate.Sheath != dbcitem->SheatheType)
            {
                LOG_ERROR("sql.sql", "Item (Entry: %u) does not have a correct sheathid (%u), must be %u .", entry, itemTemplate.Sheath, dbcitem->SheatheType);
                if (enforceDBCAttributes)
            ...
            LOG_ERROR("sql.sql", "Item (Entry: %u) does not exist in item.dbc! (not correct id?).", entry);
        ...
        if (itemTemplate.Flags2 & ITEM_FLAG2_FACTION_HORDE)
        {
            if (FactionEntry const* faction = sFactionStore.LookupEntry(HORDE))
                if ((itemTemplate.AllowableRace & faction->ReputationRaceMask[0]) == 0)
                    LOG_ERROR("sql.sql", "Item (Entry: %u) has value (%u) in `AllowableRace` races, not compatible with ITEM_FLAG2_FACTION_HORDE (%u) in Flags field, item cannot be equipped or used by these races.",
                        entry, itemTemplate.AllowableRace, ITEM_FLAG2_FACTION_HORDE);

            if (itemTemplate.Flags2 & ITEM_FLAG2_FACTION_ALLIANCE)
                LOG_ERROR("sql.sql", "Item (Entry: %u) has value (%u) in `Flags2` flags (ITEM_FLAG2_FACTION_ALLIANCE) and ITEM_FLAG2_FACTION_HORDE (%u) in Flags field, this is a wrong combination.",
        ...
        else if (itemTemplate.Flags2 & ITEM_FLAG2_FACTION_ALLIANCE)
        {
            if (FactionEntry const* faction = sFactionStore.LookupEntry(ALLIANCE))
                if ((itemTemplate.AllowableRace & faction->ReputationRaceMask[0]) == 0)
                    LOG_ERROR("sql.sql", "Item (Entry: %u) has value (%u) in `AllowableRace` races, not compatible with ITEM_FLAG2_FACTION_ALLIANCE (%u) in Flags field, item cannot be equipped or used by these races.",
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " item templates in %u ms", _itemTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}

ItemTemplate const* ObjectMgr::GetItemTemplate(uint32 entry) const
{
    return Warhead::Containers::MapGetValuePtr(_itemTemplateStore, entry);
}
...
void ObjectMgr::LoadItemSetNameLocales()
{
    ...
    LOG_INFO("server.loading", ">> Loaded " UI64FMTD " Item set name locale strings in %u ms", uint64(_itemSetNameLocaleStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 vehicle template accessories. DB table `vehicle_template_accessory` is empty.");
        return;
    }
...
void ObjectMgr::LoadPetLevelInfo()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 level pet stats definitions. DB table `pet_levelstats` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!pInfoMapEntry)
            pInfoMapEntry = std::make_unique<PetLevelInfo[]>(CONF_GET_INT("MaxPlayerLevel"));
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    LOG_INFO("server.loading", "Loading Player Create Cast Spell Data...");
    {
        ...
        if (!result)
            LOG_INFO("server.loading", ">> Loaded 0 player create cast spells. DB table `playercreateinfo_cast_spell` is empty.");
        else
        {
            uint32 count = 0;
    ...
    LOG_INFO("server.loading", "Loading Player Create Level HP/Mana Data...");
    {
        ...
        if (!result)
        {
            LOG_ERROR("server.loading", ">> Loaded 0 level health/mana definitions. DB table `player_classlevelstats` is empty.");
            ABORT();
        }
        ...
        do
        {
            ...
            if (current_level > (uint8)CONF_GET_INT("MaxPlayerLevel"))
            {
                LOG_INFO("misc", "Unused (> MaxPlayerLevel in worldserver.conf) level %u in `player_classlevelstats` table, ignoring.", current_level);
                ++count;                                    // make result loading percent "expected" correct in case disabled detail mode for example.
                continue;
            }
            ...
            if (!info)
            {
                ...
                info->levelInfo = std::make_unique<PlayerClassLevelInfo[]>(CONF_GET_INT("MaxPlayerLevel"));
            }
    ...
    LOG_INFO("server.loading", "Loading Player Create Level Stats Data...");
    {
        ...
        do
        {
            ...
            {
                if (!info->levelInfo)
                    info->levelInfo = std::make_unique<PlayerLevelInfo[]>(CONF_GET_INT("MaxPlayerLevel"));
...
void ObjectMgr::LoadQuests()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 quests definitions. DB table `quest_template` is empty.");
        return;
    }
    ...
    for (QuestLoaderHelper const& loader : QuestLoaderHelpers)
    {
        ...
        if (!result)
            LOG_INFO("server.loading", ">> Loaded 0 quest %s. DB table `%s` is empty.", loader.TableDesc, loader.TableName);
        else
        {
            do
            {
    ...
    for (auto& questPair : _questTemplates)
    {
        ...
        if (qinfo->_requiredClasses)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Quest %u does not contain any playable classes in `RequiredClasses` (%u), value set to 0 (all classes).", qinfo->GetQuestId(), qinfo->_requiredClasses);
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 instance encounters, table is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, skipped!", entry, dungeonEncounter->Name[0], lastEncounterDungeon);
            continue;
        }
        ...
        if (lastEncounterDungeon)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", entry, dungeonEncounter->Name[0], itr->second->ID, itr->second->Name[0]);
                continue;
            }
        ...
        switch (creditType)
        {
            case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->Name[0]);
                    continue;
                }
            ...
            case ENCOUNTER_CREDIT_CAST_SPELL:
                ...
                {
                    LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->Name[0]);
                    continue;
                }
                break;
            default:
                LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", creditType, entry, dungeonEncounter->Name[0]);
                continue;
        }
            ...
            if (gOption.BroadcastTextID)
            {
                if (BroadcastText const* bcText = sObjectMgr->GetBroadcastText(gOption.BroadcastTextID))
                {
                    if (bcText->Text[DEFAULT_LOCALE] != gOption.Text_0)
                        LOG_ERROR("sql.sql", "Row %u in table `npc_text` has mismatch between text%u_0 and the corresponding Text in `broadcast_text` row %u", id, i, gOption.BroadcastTextID);
                    if (bcText->Text1[DEFAULT_LOCALE] != gOption.Text_1)
                        LOG_ERROR("sql.sql", "Row %u in table `npc_text` has mismatch between text%u_1 and the corresponding Text1 in `broadcast_text` row %u", id, i, gOption.BroadcastTextID);
                }
                else
                {
...
{
    return Warhead::Containers::MapGetValuePtr(_questTemplates, quest_id);
}
...
WorldSafeLocsEntry const* ObjectMgr::GetClosestGraveyard(float x, float y, float z, uint32 MapId, uint32 team) const
{
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (!entry)
        {
            LOG_ERROR("sql.sql", "Table `graveyard_zone` has record for not existing graveyard (WorldSafeLocsID %u), skipped.", data.safeLocId);
            continue;
        }
...
uint32 ObjectMgr::GenerateGameObjectSpawnId()
{
    ...
    {
        LOG_ERROR("misc", "GameObject spawn id overflow!! Can't continue, shutting down server. Search on forum for TCE00007 for more info. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
...
void ObjectMgr::LoadGameObjectTemplate()
{
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " game object templates in %u ms", _gameObjectTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
void ObjectMgr::LoadGameObjectTemplateAddons()
{
    ...
    do
    {
        ...
        for (uint32 i = 0; i < gameObjectAddon.artKits.size(); ++i)
        {
            ...
            {
                LOG_ERROR("sql.sql", "GameObject (Entry: %u) has invalid `artkit%d` (%d) defined, set to zero instead.", entry, i, artKitID);
                continue;
            }
...
void ObjectMgr::LoadGameObjectOverrides()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 gameobject faction and flags overrides. DB table `gameobject_overrides` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!goData)
        {
            LOG_ERROR("sql.sql", "GameObject (SpawnId: %u) does not exist but has a record in `gameobject_overrides`", spawnId);
            continue;
        }
        ...
        if (gameObjectOverride.Faction && !sFactionTemplateStore.LookupEntry(gameObjectOverride.Faction))
            LOG_ERROR("sql.sql", "GameObject (SpawnId: %u) has invalid faction (%u) defined in `gameobject_overrides`.", spawnId, gameObjectOverride.Faction);

        ++count;
    } while (result->NextRow());

    LOG_INFO("server.loading", ">> Loaded %u gameobject faction and flags overrides in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
void ObjectMgr::LoadExplorationBaseXP()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 BaseXP definitions. DB table `exploration_basexp` is empty.");
        return;
    }
...
void ObjectMgr::LoadReputationOnKill()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 creature award reputation definitions. DB table `creature_onkill_reputation` is empty.");
        return;
    }
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 Points of Interest definitions. DB table `points_of_interest` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!Warhead::IsValidMapCoord(pointOfInterest.PositionX, pointOfInterest.PositionY))
        {
            LOG_ERROR("sql.sql", "Table `points_of_interest` (ID: %u) have invalid coordinates (X: %f Y: %f), ignored.", id, pointOfInterest.PositionX, pointOfInterest.PositionY);
            continue;
        }
...
void ObjectMgr::LoadQuestPOI()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 quest POI definitions. DB table `quest_poi` is empty.");
        return;
    }
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " quest POI definitions in %u ms", _questPOIStore.size(), GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
}
...
void ObjectMgr::LoadNPCSpellClickSpells()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 spellclick spells. DB table `npc_spellclick_spells` is empty.");
        return;
    }
    ...
    for (auto& creatureTemplatePair : _creatureTemplateStore)
    {
        ...
        {
            LOG_ERROR("sql.sql", "npc_spellclick_spells: Creature template %u has UNIT_NPC_FLAG_SPELLCLICK but no data in spellclick table! Removing flag", creatureTemplatePair.first);
...
void ObjectMgr::LoadQuestRelationsHelper(QuestRelations& map, std::string const& table)
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 quest relations from `%s`, table is empty.", table.c_str());
        return;
    }
...
bool ObjectMgr::LoadTrinityStrings()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 trinity strings. DB table `trinity_string` is empty. You have imported an incorrect database for more info search for TCE00003 on forum.");
        LOG_INFO("server.loading", "");
        return false;
    }
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " trinity strings in %u ms", _trinityStringStore.size(), GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", "");
    return true;
}
...
char const* ObjectMgr::GetTrinityString(uint32 entry, LocaleConstant locale) const
{
    ...
    LOG_ERROR("sql.sql", "Warhead string entry %u not found in DB.", entry);
    return "<error>";
}
...
void ObjectMgr::LoadFishingBaseSkillLevel()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 areas for fishing base skill level. DB table `skill_fishing_base_level` is empty.");
        return;
    }
...
void ObjectMgr::LoadGameTele()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 GameTeleports. DB table `game_tele` is empty!");
        return;
    }
...
void ObjectMgr::LoadMailLevelRewards()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 level dependent mail rewards. DB table `mail_level_reward` is empty.");
        return;
    }
...
void ObjectMgr::LoadTrainers()
{
    ...
    if (QueryResult trainerLocalesResult = WorldDatabase.Query("SELECT Id, locale, Greeting_lang FROM trainer_locale"))
    {
        do
        {
            ...
            if (Trainer::Trainer* trainer = Warhead::Containers::MapGetValuePtr(_trainers, trainerId))
...
void ObjectMgr::LoadCreatureDefaultTrainers()
{
    ...
    if (QueryResult result = WorldDatabase.Query("SELECT CreatureId, TrainerId FROM creature_default_trainer"))
    {
        do
        {
            ...
            Trainer::Trainer* trainer = Warhead::Containers::MapGetValuePtr(_trainers, trainerId);
            if (!trainer)
            {
...
void ObjectMgr::LoadGossipMenu()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 gossip_menu IDs. DB table `gossip_menu` is empty!");
        return;
    }
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 gossip_menu_option IDs. DB table `gossip_menu_option` is empty!");
        return;
    }
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 item_id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, Player* player, std::set<uint32>* /*skip_vendors*/, uint32 /*ORnpcflag*/) const
{
    ...
    if (ExtendedCost && !sItemExtendedCostStore.LookupEntry(ExtendedCost))
    {
        if (player)
            ...
            LOG_ERROR("sql.sql", "Table `(game_event_)npc_vendor` has Item (Entry: %u) with wrong ExtendedCost (%u) for vendor (%u), ignore", item_id, ExtendedCost, vendor_entry);
        return false;
    }
...
void ObjectMgr::LoadScriptNames()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded empty set of Script Names!");
        return;
    }
    ...
    for (auto const& creatureTemplatePair : _creatureTemplateStore)
    {
        for (uint16 lvl = creatureTemplatePair.second.minlevel; lvl <= creatureTemplatePair.second.maxlevel; ++lvl)
        {
            if (!_creatureBaseStatsStore.count(MAKE_PAIR16(lvl, creatureTemplatePair.second.unit_class)))
                LOG_ERROR("sql.sql", "Missing base stats for creature class %u level %u", creatureTemplatePair.second.unit_class, lvl);
        }
    }
...
void ObjectMgr::LoadFactionChangeAchievements()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 faction change achievement pairs. DB table `player_factionchange_achievement` is empty.");
        return;
    }
...
void ObjectMgr::LoadFactionChangeQuests()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 faction change quest pairs. DB table `player_factionchange_quests` is empty.");
        return;
    }
...
void ObjectMgr::LoadFactionChangeSpells()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 faction change spell pairs. DB table `player_factionchange_spells` is empty.");
        return;
    }
...
{
    return Warhead::Containers::MapGetValuePtr(_gameObjectTemplateStore, entry);
}
...
{
    return Warhead::Containers::MapGetValuePtr(_gameObjectOverrideStore, spawnId);
}

CreatureTemplate const* ObjectMgr::GetCreatureTemplate(uint32 entry) const
{
    return Warhead::Containers::MapGetValuePtr(_creatureTemplateStore, entry);
}

QuestPOIWrapper const* ObjectMgr::GetQuestPOIWrapper(uint32 questId) const
{
    return Warhead::Containers::MapGetValuePtr(_questPOIStore, questId);
}
