...
UPDATE `server_string` SET `content_default` = 'Item \'%u\' (type %u) not found in database.' WHERE `entry`=207;
UPDATE `server_string` SET `content_default` = 'Item \'%u\' (with extended cost %u, type %u) already in vendor list' WHERE `entry`=210;
