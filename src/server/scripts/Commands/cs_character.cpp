...
public:
    ...
    */
        ...
        if (newNameStr)
        {
            ...
            if (WorldSession* session = handler->GetSession())
            {
                if (Player* player = session->GetPlayer())
                    LOG_GM(session->GetAccountId(), "GM %s (Account: %u) forced rename %s to player %s (Account: %u)", player->GetName().c_str(), session->GetAccountId(), newName.c_str(), playerOldName.c_str(), sCharacterCache->GetCharacterAccountIdByGuid(targetGuid));
            }
            else
                LOG_GM(0, "CONSOLE forced rename '%s' to '%s' (%s)", playerOldName.c_str(), newName.c_str(), targetGuid.ToString().c_str());
        }
        else
        {
            if (target)
    ...
    static bool HandleCharacterChangeAccountCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string logString = Warhead::StringFormat("changed ownership of player %s (%s) from account %u to account %u", targetName.c_str(), targetGuid.ToString().c_str(), oldAccountId, newAccountId);
        if (WorldSession* session = handler->GetSession())
        {
            if (Player* player = session->GetPlayer())
                LOG_GM(session->GetAccountId(), "GM %s (Account: %u) %s", player->GetName().c_str(), session->GetAccountId(), logString.c_str());
        }
        else
            LOG_GM(0, "%s %s", handler->GetTrinityString(LANG_CONSOLE), logString.c_str());
        return true;
    }
