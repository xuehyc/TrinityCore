...
void Vehicle::Uninstall()
{
    ...
    {
        LOG_ERROR("entities.vehicle", "Vehicle %s attempts to uninstall, but already has STATUS_UNINSTALLING! "
...
void Vehicle::Reset(bool evading /*= false*/)
{
    ...
    LOG_DEBUG("entities.vehicle", "Vehicle::Reset (Entry: %u, %s, DBGuid: %u)", GetCreatureEntry(), _me->GetGUID().ToString().c_str(), _me->ToCreature()->GetSpawnId());
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (_status == STATUS_UNINSTALLING)
    {
        LOG_ERROR("entities.vehicle", "Passenger %s, attempting to board vehicle %s during uninstall! SeatId: %d",
            unit->GetGUID().ToString().c_str(), _me->GetGUID().ToString().c_str(), (int32)seatId);
        return false;
    }

    LOG_DEBUG("entities.vehicle", "Unit %s scheduling enter vehicle (entry: %u, vehicleId: %u, guid: %s on seat %d",
...
Vehicle* Vehicle::RemovePassenger(Unit* unit)
{
    ...
    LOG_DEBUG("entities.vehicle", "Unit %s exit vehicle entry %u id %u guid %s seat %d",
...
void VehicleJoinEvent::Abort(uint64)
{
    ...
    if (Target)
    {
        LOG_DEBUG("entities.vehicle", "Passenger %s, board on vehicle %s SeatId: %d cancelled",
        ...
        LOG_DEBUG("entities.vehicle", "Passenger %s, board on uninstalled vehicle SeatId: %d cancelled",
