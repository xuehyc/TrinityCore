...
void Log::CreateAppenderFromConfig(const char* name)
{
    ...
    if (tokens.size() < 2)
    {
        fprintf(stdout, "Log::CreateAppenderFromConfig: Wrong configuration for appender %s. Config line: %s\n", name, options.c_str());
        return;
    }
    ...
    {
        fprintf(stdout, "Log::CreateAppenderFromConfig: Wrong Log Level %u for appender %s\n", level, name);
        return;
    }
    ...
    switch (type)
    {
        ...
        default:
            fprintf(stdout, "Log::CreateAppenderFromConfig: Unknown type %u for appender %s\n", type, name);
            break;
    }
}
...
void Log::CreateLoggerFromConfig(const char* name)
{
    ...
    {
        fprintf(stdout, "Log::CreateLoggerFromConfig: Wrong type %u for logger %s\n", type, name);
        return;
    }
    ...
    {
        fprintf(stdout, "Error while configuring Logger %s. Already defined\n", name);
        return;
    }
    ...
    if (level > LOG_LEVEL_FATAL)
    {
        fprintf(stdout, "Log::CreateLoggerFromConfig: Wrong Log Level %u for logger %s\n", type, name);
        return;
    }
    ...
    while (ss)
    {
        if (Appender* appender = GetAppenderByName(str))
        {
            ...
            fprintf(stdout, "Error while configuring Appender %s in Logger %s. Appender does not exist", str.c_str(), name);
        ss >> str;
    }
}
