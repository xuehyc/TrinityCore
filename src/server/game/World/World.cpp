...
void World::LoadConfigSettings(bool reload)
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "WORLD: VMap support included. LineOfSight:%i, getHeight:%i, indoorCheck:%i PetLOS:%i", enableLOS, enableHeight, enableIndoor, enablePetLOS);
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "WORLD: VMap data directory is: %svmaps", m_dataPath.c_str());
    ...
    // IRC LINES
    sIRC.ILINES[WOW_IRC] = ConfigMgr::GetStringDefault("chat.wow_irc", "$GUID [\003$Level|\002$Name\002\003]: $Msg");
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Updating preserved accounts...");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Remove entries from castle log older than a week...");
    ...
    sLog->outInfo(LOG_FILTER_WORLDSERVER, "WORLD: World initialized in %u minutes %u seconds", (startupDuration / 60000), ((startupDuration % 60000) / 1000));
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_UPTIME].Passed())
    {
        ...
        LoginDatabase.PExecute("UPDATE realmlist SET flag = 0 WHERE id = %u AND flag != 0", realmID);
    }
...
void World::LoadCharacterNameData()
{
    ...
    if (!result)
    {
        sLog->outError(LOG_FILTER_SERVER_LOADING, "No character name data loaded, empty query");
        return;
    }
