            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_START_REVIVE:
                    {
                        ...
                        creatures.remove_if(Server::AnyDeadUnitCheck());
                        creatures.remove_if(Server::UnitAuraCheck(true, SPELL_OHGAN_ORDERS_TRIGGER));
                        Server::Containers::RandomResize(creatures, 1);
                        if (creatures.empty())
                            return;
