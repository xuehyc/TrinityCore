...
public:
    ...
    static bool HandleServerDebugCommand(ChatHandler* handler, char const* /*args*/)
    {
...

        {
            ...
            if (dbPort)
                dbPortOutput = Server::StringFormat("Realmlist (Realm Id: %u) configured in port %" PRIu16, realm.Id.Realm, dbPort);
            else
                dbPortOutput = Server::StringFormat("Realm Id: %u not found in `realmlist` table. Please check your setup", realm.Id.Realm);
        }
