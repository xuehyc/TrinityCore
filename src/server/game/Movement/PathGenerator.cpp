    ...
    _navMeshQuery(nullptr)
{
    ...
    LOG_DEBUG("maps.mmaps", "++ PathGenerator::PathGenerator for %s", _source->GetGUID().ToString().c_str());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest)
{
    ...
    WH_METRIC_DETAILED_EVENT("mmap_events", "CalculatePath", "");
    ...
    LOG_DEBUG("maps.mmaps", "++ PathGenerator::CalculatePath() for %s", _source->GetGUID().ToString().c_str());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                LOG_ERROR("maps.mmaps", "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            LOG_ERROR("maps.mmaps", "Path Build failed\n%s", _source->GetDebugInfo().c_str());
        }
    ...
    else
    {
        ...
        if (!_polyLength || dtStatusFailed(dtResult))
        {
            ...
            LOG_ERROR("maps.mmaps", "%s Path Build failed: 0 length path", _source->GetGUID().ToString().c_str());
            BuildShortcut();
            _type = PATHFIND_NOPATH;
            return;
        }
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    else if ( pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        LOG_DEBUG("maps.mmaps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
    ...
    *smoothPathSize = 0;
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        if (dtStatusFailed(_navMeshQuery->getPolyHeight(polys[0], result, &result[1])))
            LOG_DEBUG("maps.mmaps", "Cannot find height at position X: %f Y: %f Z: %f for %s", result[2], result[0], result[1], _source->GetDebugInfo().c_str());
        result[1] += 0.5f;
...
void PathGenerator::ShortenPathUntilDist(G3D::Vector3 const& target, float dist)
{
    ...
    {
        LOG_ERROR("maps.mmaps", "PathGenerator::ReducePathLengthByDist called before path was successfully built");
        return;
    }
