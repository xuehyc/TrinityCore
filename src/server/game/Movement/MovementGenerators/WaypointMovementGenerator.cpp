...
void WaypointMovementGenerator<Creature>::DoInitialize(Creature* owner)
{
    ...
    if (!_path)
    {
        LOG_ERROR("sql.sql", "WaypointMovementGenerator::DoInitialize: couldn't load path for creature (%s) (_pathId: %u)", owner->GetGUID().ToString().c_str(), _pathId);
        return;
    }
...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* owner)
{
    ...
    {
        LOG_DEBUG("maps.script", "Creature movement start script %u at point %u for %s.", waypoint.eventId, _currentNode, owner->GetGUID().ToString().c_str());
        owner->ClearUnitState(UNIT_STATE_ROAMING_MOVE);
