...
public:
    ...
    static bool HandleRBACPermListCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (granted.empty())
            handler->PSendSysMessage("%s", handler->GetWarheadString(LANG_RBAC_LIST_EMPTY));
        else
        {
    ...
    static bool HandleRBACListPermissionsCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!id)
        {
            ...
            handler->PSendSysMessage("%s", handler->GetWarheadString(LANG_RBAC_LIST_PERMISSIONS_HEADER));
        ...
        else
        {
            ...
            handler->PSendSysMessage("%s", handler->GetWarheadString(LANG_RBAC_LIST_PERMISSIONS_HEADER));
            handler->PSendSysMessage(LANG_RBAC_LIST_ELEMENT, permission->GetId(), permission->GetName().c_str());
            handler->PSendSysMessage("%s", handler->GetWarheadString(LANG_RBAC_LIST_PERMS_LINKED_HEADER));
