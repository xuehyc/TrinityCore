...
GAME_API std::atomic<bool> World::m_stopEvent(false);
GAME_API uint8 World::m_ExitCode = SHUTDOWN_EXIT_CODE;

GAME_API std::atomic<uint32> World::m_worldLoopCounter(0);

GAME_API float World::m_MaxVisibleDistanceOnContinents = DEFAULT_VISIBILITY_DISTANCE;
GAME_API float World::m_MaxVisibleDistanceInInstances  = DEFAULT_VISIBILITY_INSTANCE;
GAME_API float World::m_MaxVisibleDistanceInBGArenas   = DEFAULT_VISIBILITY_BGARENAS;

GAME_API int32 World::m_visibility_notify_periodOnContinents = DEFAULT_VISIBILITY_NOTIFY_PERIOD;
GAME_API int32 World::m_visibility_notify_periodInInstances  = DEFAULT_VISIBILITY_NOTIFY_PERIOD;
GAME_API int32 World::m_visibility_notify_periodInBGArenas   = DEFAULT_VISIBILITY_NOTIFY_PERIOD;
...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Welcome to World of Warcraft."));
    ...
    auto setRegenRate = [this](Rates rate, char const* configKey)
    {
        ...
        {
            LOG_ERROR("server.loading", "%s (%f) must be > 0. Using 1 instead.", configKey, rate_values[rate]);
    ...
    {
        LOG_ERROR("server.loading", "DemonHuntersPerRealm (%i) must be in range 0..16. Set to 1.", m_int_configs[CONFIG_DEMON_HUNTERS_PER_REALM]);
    ...
    {
        LOG_ERROR("server.loading", "StartDemonHunterPlayerLevel (%i) must be in range 98..MaxPlayerLevel(%u). Set to 98.",
    ...
    {
        LOG_ERROR("server.loading", "StartDemonHunterPlayerLevel (%i) must be in range 1..MaxPlayerLevel(%u). Set to %u.",
    ...
    {
        LOG_ERROR("server.loading", "Currency.StartApexisCrystals (%i) must be >= 0, set to default 0.", m_int_configs[CONFIG_CURRENCY_START_APEXIS_CRYSTALS]);
    ...
    {
        LOG_ERROR("server.loading", "Currency.MaxApexisCrystals (%i) can't be negative. Set to default 20000.", m_int_configs[CONFIG_CURRENCY_MAX_APEXIS_CRYSTALS]);
    ...
    {
        LOG_ERROR("server.loading", "Currency.StartArtifactKnowledge (%i) must be >= 0, set to default 0.", m_int_configs[CONFIG_CURRENCY_START_ARTIFACT_KNOWLEDGE]);
    ...
    {
        LOG_ERROR("server.loading", "MinPetitionSigns (%i) must be in range 0..4. Set to 4.", m_int_configs[CONFIG_MIN_PETITION_SIGNS]);
    ...
    {
        LOG_WARN("server.loading", "SaveRespawnTimeImmediately triggers assertions when disabled, overridden to Enabled");
    ...
    if (int32 hotfixCacheId = sConfigMgr->GetIntDefault("HotfixCacheVersion", 0))
    {
        ...
        if (hotfixCacheId > 0)
            ...
            LOG_ERROR("server.loading", "HotfixCacheVersion can't be negative %d, ignored.", hotfixCacheId);
    }
    LOG_INFO("server.loading", "Hotfix cache version set to: %u", m_int_configs[CONFIG_HOTFIX_CACHE_VERSION]);
...
#if SERVER_PLATFORM == SERVER_PLATFORM_UNIX || SERVER_PLATFORM == SERVER_PLATFORM_APPLE
    if (dataPath[0] == '~')
    {
    ...
    if (m_bool_configs[CONFIG_START_ALL_SPELLS])
        LOG_WARN("server.loading", "PlayerStart.AllSpells enabled - may not function as intended!");
...
void World::SetInitialWorldSettings()
{
    ...
    LOG_INFO("server.loading", "Initialize PlayerDump...");
    ...
    LOG_INFO("server.loading", "Loading server strings...");
    if (!sObjectMgr->LoadServerStrings())
    ...
    LOG_INFO("server.loading", "Loading PetFamilySpellsStore Data...");
    sSpellMgr->LoadPetFamilySpellsStore();

    LOG_INFO("server.loading", "Loading Spell Totem models...");
    ...
    LOG_INFO("server.loading", "Loading Creature template scaling...");
    ...
    LOG_INFO("server.loading", "Loading AreaTrigger Templates...");
    sAreaTriggerDataStore->LoadAreaTriggerTemplates();

    LOG_INFO("server.loading", "Loading Conversation Templates...");
    sConversationDataStore->LoadConversationTemplates();

    LOG_INFO("server.loading", "Loading Scenes Templates...");
    sObjectMgr->LoadSceneTemplates();

    LOG_INFO("server.loading", "Loading Player Choices...");
    sObjectMgr->LoadPlayerChoices();

    LOG_INFO("server.loading", "Loading Player Choices Locales...");
    ...
    LOG_INFO("server.loading", "Loading Criteria Modifier trees...");
    sCriteriaMgr->LoadCriteriaModifiersTree();
    LOG_INFO("server.loading", "Loading Criteria Lists...");
    sCriteriaMgr->LoadCriteriaList();
    LOG_INFO("server.loading", "Loading Criteria Data...");
    ...
    {
        LOG_INFO("server.loading", "Loading Black Market Templates...");
        sBlackMarketMgr->LoadTemplates();

        LOG_INFO("server.loading", "Loading Black Market Auctions...");
    ...
    LOG_INFO("server.loading", "Loading mount definitions...");
    CollectionMgr::LoadMountDefinitions();

    LOG_INFO("server.loading", "Loading GM bugs...");
    sSupportMgr->LoadBugTickets();

    LOG_INFO("server.loading", "Loading GM complaints...");
    sSupportMgr->LoadComplaintTickets();

    LOG_INFO("server.loading", "Loading GM suggestions...");
    sSupportMgr->LoadSuggestionTickets();

    /*LOG_INFO("server.loading", "Loading GM surveys...");
    sSupportMgr->LoadSurveys();*/

    LOG_INFO("server.loading", "Loading garrison info...");
    ...
    LOG_INFO("server.loading", "Loading race and class expansion requirements...");
    sObjectMgr->LoadRaceAndClassExpansionRequirements();

    LOG_INFO("server.loading", "Loading character templates...");
    sCharacterTemplateDataStore->LoadCharacterTemplates();

    LOG_INFO("server.loading", "Loading realm names...");
    sObjectMgr->LoadRealmNames();

    LOG_INFO("server.loading", "Loading battle pets info...");
    BattlePetMgr::Initialize();

    LOG_INFO("server.loading", "Loading scenarios");
    sScenarioMgr->LoadDB2Data();
    sScenarioMgr->LoadDBData();

    LOG_INFO("server.loading", "Loading scenario poi data");
...
void World::LoadAutobroadcasts()
{
    ...
    LOG_INFO("server.loading", ">> Loaded " SZFMTD " autobroadcast definitions in %u ms", m_Autobroadcasts.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
namespace Server
{
    class WorldWorldTextBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetServerString(i_textId, locale);
...
void World::SendWorldText(uint32 string_id, ...)
{
    ...
    Server::WorldWorldTextBuilder wt_builder(string_id, &ap);
    Server::LocalizedPacketListDo<Server::WorldWorldTextBuilder> wt_do(wt_builder);
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
void World::SendAutoBroadcast()
{
    ...
    auto itr = Server::Containers::SelectRandomWeightedContainerElement(m_Autobroadcasts, [](AutobroadcastContainer::value_type const& pair)
    {
    ...
    LOG_DEBUG("misc", "AutoBroadcast: '%s'", itr->second.Message.c_str());
}
...
{
    LOG_INFO("misc", "Daily quests reset for all characters.");
...
{
    LOG_INFO("misc", "Weekly quests reset for all characters.");
...
{
    LOG_INFO("misc", "Monthly quests reset for all characters.");
