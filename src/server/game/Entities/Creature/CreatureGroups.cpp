...
void FormationMgr::AddCreatureToGroup(ObjectGuid::LowType leaderSpawnId, Creature* creature)
{
    ...
    if (itr != map->CreatureGroupHolder.end())
    {
        ...
        LOG_DEBUG("entities.unit", "Group found: %u, inserting creature %s, Group InstanceID %u", leaderSpawnId, creature->GetGUID().ToString().c_str(), creature->GetInstanceId());
        ...
        auto bounds = Warhead::Containers::MapEqualRange(map->GetCreatureBySpawnIdStore(), creature->GetSpawnId());
    ...
    {
        //Create new group
        LOG_DEBUG("entities.unit", "Group not found: %u. Creating new group.", leaderSpawnId);
...
{
    LOG_DEBUG("entities.unit", "Deleting member pointer to GUID: %u from group %u", group->GetLeaderSpawnId(), member->GetSpawnId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // check data correctness
        {
            ...
            {
                LOG_ERROR("sql.sql", "creature_formations table leader guid %u incorrect (not exist)", member.LeaderSpawnId);
                continue;
            }

            if (!sObjectMgr->GetCreatureData(memberSpawnId))
            {
                LOG_ERROR("sql.sql", "creature_formations table member guid %u incorrect (not exist)", memberSpawnId);
                continue;
            }
    ...
    for (ObjectGuid::LowType leaderSpawnId : leaderSpawnIds)
    {
        ...
        {
            LOG_ERROR("sql.sql", "creature_formation contains leader spawn %u which is not included on its formation, removing", leaderSpawnId);
...
{
    return Warhead::Containers::MapGetValuePtr(_creatureGroupMap, spawnId);
}
...
void CreatureGroup::FormationReset(bool dismiss)
{
    for (auto const& pair : _members)
    {
        if (pair.first != _leader && pair.first->IsAlive())
        {
            ...
            LOG_DEBUG("entities.unit", "CreatureGroup::FormationReset: Set %s movement for member %s", dismiss ? "default" : "idle", pair.first->GetGUID().ToString().c_str());
        }
    }
