...
bool ObjectMgr::LoadTrinityStrings()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded " SZFMTD " arkcore strings in %u ms", _trinityStringStore.size(), GetMSTimeDiffToNow(oldMSTime));
    return true;
}
...
char const* ObjectMgr::GetTrinityString(uint32 entry, LocaleConstant locale) const
{
    ...
    TC_LOG_ERROR("sql.sql", "Arkcore string entry %u not found in DB.", entry);
    return "<error>";
}
